...
namespace mxnet {
  namespace op {
    ...
    DType get_coordinate_weight_cpu(DType argmax_h, DType argmax_w,
    ...
    void deformable_upsampling_forward(
      ...
      for (int idx = 0; idx < num_threads; ++idx) {
        ...
        // old version which ignores (-1. 0] positions
//        if (im_y >= 0 && im_x >= 0 && im_y < height && im_x < width) {
//          val = bilinear_interpolate_cpu(
//              data_ptr,
//              height,
//              width,
...
//        }
    ...
    void deformable_upsampling_backward_data(
      ...
      for (int idx = 0; idx < num_threads; ++idx) {
        ...
        bilinear_interpolate_gradient_cpu<DType>(
            height,
            width,
            im_y, im_x,
            &w1, &w2, &w3, &w4,
        ...
        if (x_low >= 0 && x_high >= 0 && y_low >= 0 && y_high >= 0) {
          ...
          add(data_diff_ptr + y_low * width + x_low, g1);
          add(data_diff_ptr + y_low * width + x_high, g2);
          add(data_diff_ptr + y_high * height + x_low, g3);
          add(data_diff_ptr + y_high * height + x_high, g4);
        }
        //don't update offset in this loop, because it will cause atomic wait in cuda,
        // since offset is shared among threads
      }
    }
    ...
    void deformable_upsampling_backward_offset(
      ...
      for (int idx = 0; idx < num_threads; ++idx) {
        ...
        for (int in_c = cstart; in_c < cend; ++in_c) {
          ...
          DType weight = get_coordinate_weight_cpu<DType>(im_y, im_x, height, width, offset_data, bp_dir);

          add(offset_diff + idx, weight * this_diff);
